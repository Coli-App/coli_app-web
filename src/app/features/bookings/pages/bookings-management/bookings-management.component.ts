...
export class BookingsManagementComponent implements OnInit {
  ...
  loadBookings(): void {
    ...
    this.bookingsService.getBookingsByCreator(userId).subscribe({
      next: (bookings) => {
  ...
  private performCancelBooking(booking: BookingWithSpace): void {
    this.bookingsService.deleteBooking(booking.id.toString()).subscribe({
      next: () => {
        this.spacesService.getSpaceById(booking.espacio_id).subscribe({
          next: (spaceData) => {
            ...
            this.spacesService.updateSpace(booking.espacio_id, updateSpace).subscribe({
              next: () => {
